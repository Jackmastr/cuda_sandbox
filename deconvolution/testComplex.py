...
img3 = np.array(vis_fg_pntsrc[500], dtype=np.complex128)
ker = np.ones(1024)
...
ker128 = np.array(np.fft.fft(ker), dtype=np.complex128)
...
class TestComplex(unittest.TestCase):
	def test_tolerances(self):
		...
		A3 = deconv.clean(img3, ker128, stop_if_div=False, tol=1e-6)[0]
		...
		B3 = clean(img3, ker128, stop_if_div=False, tol=1e-6)[0]
	...
	def test_areas(self):
		...
		A3 = deconv.clean(img3, ker128, stop_if_div=False, tol=0, area=area1)[0]
		...
		B3 = clean([img3]*3, [ker128]*3, stop_if_div=False, tol=0, area=area1)[0][1]
		for i in xrange(1024):
			self.assertEqual(A0[i], B0[i])
	...
	def test_mdls(self):
		...
		A3 = deconv.clean(img3, ker128, stop_if_div=False, tol=1e-6, mdl=np.array(area1, dtype=np.complex128))[0]
		...
		B3 = clean(img3, ker128, stop_if_div=False, tol=1e-6, mdl=np.array(area1, dtype=np.complex128))[0]
		for i in xrange(1024):
			self.assertEqual(A0[i], B0[i])
	...
	def test_gains(self):
		...
		A3 = deconv.clean(img3, ker128, stop_if_div=False, tol=1e-6, gain=0.001)[0]
