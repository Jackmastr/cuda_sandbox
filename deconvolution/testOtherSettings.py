...
class TestOtherSettings(unittest.TestCase):
	...
	def test_areas(self):
		...
		A0 = deconv.clean(img0, ker, stop_if_div=False, tol=0, area=area0, verbose=True)[0]
		# A1 = deconv.clean(img1, ker, stop_if_div=False, tol=0, area=area1, verbose=True)[0]
		# A2 = deconv.clean(img2, ker, stop_if_div=False, tol=0, area=area1, verbose=True)[0]
