...
class NotSquareTest(unittest.TestCase):
	def testLopsidedImage(self):
		...
		self.assertTrue(np.allclose(check0, mf_redux.MedianFilter(kernel_size=1, n=1, m=73, input=in0)))
		self.assertTrue(np.allclose(check1, mf_redux.MedianFilter(kernel_size=1, n=5, m=3, input=in1)))
